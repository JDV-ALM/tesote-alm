...
class TestWebhookEvent(unittest.TestCase):
    ...
    def test_check_idempotency_no_duplicate(self):
        ...
        event.search.assert_called_once_with(
            [("event_id", "=", "event_123"), ("status", "in", ["completed", "processing"])], limit=1
        )
    ...
    def test_log_processing_complete(self):
        ...
        with patch(
            "models.tesote_webhook_event.fields.Datetime.now",
...
class TestWebhookController(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env.__getitem__.return_value.sudo.return_value.search.return_value = (
            self.webhook_config
        )
