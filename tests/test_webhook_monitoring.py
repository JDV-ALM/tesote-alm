...
def test_webhook_monitor_update_metrics():
    ...
    metrics_by_type = {'sync.updates_available': {
        'received': 1,
        'processed': 1,
        'failed': 0,
        'avg_time': 150.0
...
def test_webhook_monitor_success_rate():
    ...
    monitor.total_received = 0
    monitor.total_processed = 0
    success_rate = 0.0 if monitor.total_received == 0 else (monitor.total_processed / monitor.total_received) * 100
...
def test_webhook_event_statistics():
    ...
    for i in range(100):
        ...
        event.sync_job_id = f'job_{i}' if event.event_type == 'sync.updates_available' and event.status == 'completed' else None
        events.append(event)
    ...
    stats['syncs_completed'] = len([e for e in sync_events if e.status == 'completed' and e.sync_job_id])
