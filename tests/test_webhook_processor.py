...
import tests.conftest  # This sets up all the mocks
...
class TestWebhookProcessor(unittest.TestCase):
    ...
    def test_process_event_success(self):
        ...
        account.sync_transactions = Mock()
        ...
        # The sync_transactions is called either directly or via with_delay
        if hasattr(account, 'with_delay'):
            account.with_delay.assert_called()
        else:
            account.sync_transactions.assert_called_once()
    ...
    def test_handle_sync_updates_with_delay(self):
        ...
        account.with_delay.return_value.sync_transactions.return_value = job
        
        self.account_model.search.return_value = account
        
        self.processor._handle_sync_updates(self.webhook_event, payload_data)
        
        account.with_delay.assert_called_once()
    ...
    def test_handle_account_created(self):
        ...
        new_account = Mock(spec=['id', 'name', 'sync_transactions'])  # Use spec to limit attributes
        new_account.id = 3
        new_account.name = "New Account"
        new_account.sync_transactions = Mock()
        ...
        # Since sync_required is True and no with_delay, sync should be called directly
        new_account.sync_transactions.assert_called_once()
    ...
    def test_handle_account_updated(self):
        ...
        account.with_delay = Mock()
        account.with_delay.return_value.sync_transactions = Mock()
    ...
    def test_prepare_account_values(self):
        ...
        self.assertEqual(values['external_id'], 'acc_test')
    ...
    def test_prepare_transaction_values(self):
        ...
        self.assertEqual(values['external_id'], 'tr_test')
    ...
    def test_fetch_and_create_account(self):
        ...
        with patch('components.adapter.TesoteAdapter') as MockAdapter:
            ...
            self.assertEqual(create_vals['external_id'], 'acc_fetch')
