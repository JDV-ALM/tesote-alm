...
class TesoteAdapterStandalone:
    ...
    def sync_transactions(self, tesote_account_id, cursor=None, count=100):
        ...
        print("=== SYNC TRANSACTIONS REQUEST ===")
...
def test_adapter_basic():
    ...
    adapter = TesoteAdapterStandalone(
        api_url="https://equipo.tesote.com", api_token="test_token_123"
    )
    ...
    assert (
        adapter._get_url("account_detail", account_id="123")
        == "https://equipo.tesote.com/api/v2/accounts/123"
    )
...
def test_api_calls():
    ...
    responses.add(
        responses.GET, "https://equipo.tesote.com/api/v2/status", json={"status": "ok"}, status=200
    )
...
def test_sync_logging():
    ...
    print(
        f"  Final results: +{log_entry['records_added']} ~{log_entry['records_modified']} -{log_entry['records_removed']}"
    )
...
def test_background_sync_simulation():
    ...
    for i, step in enumerate(steps, 1):
        ...
        if "Syncing transactions" in step:
            print("    → +3 transactions added")
        elif "Importing accounts" in step:
            print("    → 2 accounts imported")
...
def test_auto_sync_configuration():
    ...
    backend_config = {"auto_sync_enabled": False, "sync_interval_hours": 24, "active": True}
