...
class TesoteWebhookMonitor(models.Model):
    ...
    metrics_by_type = fields.Text(
        string='Metrics by Type',
        help='JSON containing metrics broken down by event type'
    )
    ...
    def update_metrics(self, webhook_event):
        ...
        if webhook_event.processing_duration:
            if monitor.min_processing_time == 0 or webhook_event.processing_duration < monitor.min_processing_time:
            ...
            monitor.avg_processing_time = (current_total + webhook_event.processing_duration) / total_processed
        ...
        if webhook_event.processing_duration:
            ...
            metrics_by_type[event_type]['avg_time'] = (
                (current_avg * (count - 1) + webhook_event.processing_duration) / count
            )
    ...
    def send_alert_notifications(self, alerts):
        ...
        for alert in alerts:
            backend = self.env['tesote.backend'].search([
                ('name', '=', alert['backend'])
            ], limit=1)
            ...
            _logger.info('Alert notification sent to %s for %s', config.alert_email_to, alert['type'])
