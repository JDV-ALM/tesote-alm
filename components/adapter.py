...
class TesoteAdapter:
        ...
        **kwargs
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                if response.status_code == 403 and ('HISTORY_SYNC_FORBIDDEN' in str(error_code) or 
                                                     'Historical sync is disabled' in str(error_message)):
                    _logger.warning(
                        f"Historical sync forbidden. Will skip historical data and sync from now."
                    )
    ...
    def sync_transactions(
        ...
        _logger.info(
            f"Sync result: {added} added, {modified} modified, "
            f"{removed} removed transactions"
        )
