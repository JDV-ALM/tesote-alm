...
class MigrationRunner:
    ...
    def get_executed_migrations(self):
        ...
        self.cr.execute(f"SELECT version FROM {self.schema_table} WHERE direction = 'up' ORDER BY version")
    ...
    def get_available_migrations(self):
        ...
        for migration_file in sorted(self.migrations_path.rglob("*.py")):
            ...
            if version and version[0].isdigit():
                migrations.append((version, migration_file, self._get_migration_class_name(version)))
    ...
    def rollback(self, steps=1):
        ...
        available = {version: (file_path, class_name) 
        ...
        for version in to_rollback:
            if version in available:
                ...
                _logger.warning(f"Migration file for {version} not found, removing from schema table")
    ...
    def status(self):
        ...
        print("-" * 60)
        print(f"Total: {len(available)}, Executed: {len(executed)}, Pending: {len(available) - len(executed)}")
        print()
    ...
    def create_migration(self, name):
        ...
        snake_name = ''.join(['_' + c.lower() if c.isupper() and i > 0 else c.lower() 
                            for i, c in enumerate(name)]).lstrip('_')
