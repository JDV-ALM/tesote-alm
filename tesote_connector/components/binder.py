...
class TesoteBinder:
    ...
    def bind(self, external_id: str, odoo_id: int) -> None:
        ...
        if record.exists():
            ...
            _logger.debug(
                f"Bound {self.model_name} Tesote ID {external_id} "
                f"to Odoo ID {odoo_id}"
            )
    ...
    def to_external(self, binding) -> Optional[str]:
        """
    ...
    def get_or_create(
        ...
        _logger.info(
            f"Created new {self.model_name} ID {record.id} "
            f"for Tesote ID {external_id}"
        )
