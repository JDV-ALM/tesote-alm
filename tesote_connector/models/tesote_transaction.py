...
class TesoteTransaction(models.Model):
    ...
    description = fields.Text(
        ...
        help='Detailed transaction description'
    )
    
    counterparty_name = fields.Char(
        string='Counterparty',
        help='Name of the counterparty'
    )
    
    categories = fields.Char(
        string='Categories',
        help='Transaction categories (comma-separated)'
    )
    
    transaction_data = fields.Text(
        string='Transaction Data',
        help='Additional transaction data in JSON format'
    )
    
    tesote_imported_at = fields.Datetime(
        string='Imported at Tesote',
        readonly=True,
        help='Import date in Tesote system'
    )
    ...
    account_move_id = fields.Many2one(
        ...
        help='Related accounting entry'
    )
    
    is_reconciled = fields.Boolean(
        string='Reconciled',
        default=False,
        help='Whether transaction has been reconciled'
    )
    ...
    def create_journal_entry(self):
        ...
        if self.account_move_id:
            raise UserError(
                _("Journal entry already exists for this transaction")
            )
    ...
    def _parse_date(self, date_string):
        ...
        try:
            ...
            if 'T' in str(date_string):
                ...
                if '+' in str(date_string) or (len(str(date_string)) > 10 and str(date_string)[-6] == '-'):
    ...
    def _parse_datetime(self, datetime_string):
        ...
        try:
            ...
            if 'T' in str(datetime_string):
                # Remove timezone info if present
                if '+' in str(datetime_string) or (len(str(datetime_string)) > 10 and str(datetime_string)[-6] == '-'):
    ...
    def create_from_sync_data(self, account, data):
        ...
        if not currency:
            currency = self.env['res.currency'].search([
                ('name', '=', data.get('iso_currency_code', 'USD'))
            ], limit=1)
            if not currency:
